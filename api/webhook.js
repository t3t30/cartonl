// api/webhook.js — Recebe notificações do Mercado Pago e envia emails via Resend
export default async function handler(req, res) {
  if (req.method !== 'POST') return res.status(405).end();

  const RESEND_KEY = process.env.RESEND_API_KEY;
  const MP_TOKEN   = process.env.MP_ACCESS_TOKEN;
  const SEU_EMAIL  = 'contato@cartorioemcasa.com.br';

  try {
    const { type, data } = req.body;

    // MP envia vários tipos de notificação — só nos importa pagamento aprovado
    ...
    const ref        = pagamento.external_reference || '';      // "Certidão de Nascimento-SP-1234567"
    const partes     = ref.split('-');
    const tipoCert   = partes.slice(0, partes.length - 2).join('-') || 'Certidão';
            ...
            <div style="background:#f8fafc;border-radius:12px;padding:24px;border:1px solid #e2e8f0">
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td style="font-size:13px;color:#0f172a;font-weight:600;text-align:right">${estado}</td>
                </tr>
                <tr><td colspan="2" style="border-top:1px solid #e2e8f0;padding:0"></td></tr>
                <tr>
...
<body style="font-family:Arial,sans-serif;background:#f1f5fd;padding:24px">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;padding:28px;border-left:4px solid #1a3fa8">
    ...
    <table width="100%" cellpadding="6" cellspacing="0" style="font-size:14px;color:#334155">
      ...
      <tr><td style="color:#64748b">Estado</td><td><strong>${estado}</strong></td></tr>
      <tr><td style="color:#64748b">Cliente</td><td><strong>${nomeCliente}</strong></td></tr>
      <tr><td style="color:#64748b">E-mail</td><td><strong>${emailCliente}</strong></td></tr>
      ...
      <tr><td style="color:#64748b">ID do pagamento</td><td><strong>${data.id}</strong></td></tr>
    </table>
  </div>
</body>
</html>`;
    ...
    if (emailCliente && emailCliente.includes('@')) {
      await enviar(emailCliente, '✅ Pagamento confirmado — Cartório em Casa', htmlCliente);
    }

    // Email para o admin
    await enviar(SEU_EMAIL, `💰 Novo pedido pago: ${tipoCert} (${estado}) — R$ ${valorPago}`, htmlAdmin);
